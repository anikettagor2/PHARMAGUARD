...
import { ShieldCheck, Dna, User, Mail, Lock } from "lucide-react";
...
export default function Signup() {
  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center p-4 selection:bg-purple-600 relative overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-full h-full bg-[radial-gradient(circle_at_80%_20%,rgba(168,85,247,0.1),transparent_50%)]" />
      <div className="absolute bottom-0 left-0 w-full h-full bg-[radial-gradient(circle_at_20%_80%,rgba(168,85,247,0.1),transparent_50%)]" />
      ...
      >
        <div className="bg-[#0c0c0c] p-10 rounded-[40px] border border-purple-900/40 shadow-[0_0_50px_rgba(0,0,0,1)] backdrop-blur-xl">
          ...
          <h2 className="text-3xl font-black text-white text-center mb-2 tracking-tighter uppercase">Initialize <span className="text-purple-500">Node</span></h2>
          <p className="text-gray-500 text-center mb-10 text-[10px] font-black uppercase tracking-[0.3em]">Deploy Clinician Protocol</p>
          ...
          <form onSubmit={handleSignup} className="space-y-6">
            <div className="space-y-2">
              <label className="block text-[10px] font-black text-purple-400 uppercase tracking-widest ml-1">Identity Label (Full Name)</label>
              <div className="relative group">
                <User className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-purple-900 group-focus-within:text-purple-400 transition-colors" />
                <input 
                  type="text" 
                  ...
                  value={name}
                  placeholder="Clinical Practitioner"
          ...
          <div className="mt-10 pt-10 border-t border-purple-900/20 text-center">
            <p className="text-gray-600 text-[10px] font-black uppercase tracking-widest">
              Existing Protocol Member? <Link href="/login" className="text-purple-400 hover:text-white hover:underline transition-colors">SignIn</Link>
            </p>
          </div>
        </div>
        ...
        <div className="mt-8 text-center opacity-30">
          <p className="text-[8px] font-black text-gray-500 uppercase tracking-[0.8em]">Deployment Status: Ready</p>
        </div>
      </motion.div>
    </div>
  );
